...
{#if userId && isAuthenticated}
    <div class="bg-gradient-to-r from-gray-500 to-blue-700 p-6 rounded-t-lg flex justify-between items-center">
      <h1 class="text-4xl font-extrabold text-white">User Profile</h1>
      <img src="/svg/user-details.svg" alt="User Icon" class="w-12 h-12"/>
    </div>
    <div class="bg-gradient-to-r from-gray-500 to-blue-700 p-6 rounded-b-lg space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-2">
          <p class="text-lg text-gray-800"><span class="font-bold text-white">Username:</span> {username}</p>
          <p class="text-lg text-gray-800"><span class="font-bold text-white">Email:</span> {userEmail}</p>
          <p class="text-lg text-gray-800"><span class="font-bold text-white">Role:</span> {userRole}</p>
        </div>
        </div>
